...
const ChatBox = () => {
      ...
      stompClient = new Client({
        ...
        onConnect: () => {
          stompClient.subscribe(
            `/user/${chat.chat.id}/queue/messages`,
            (message) => {
              const body = JSON.parse(message.body);
              dispatch({ type: "MESSAGE_RECEIVED", payload: body });
            }
          );
        },
        onStompError: (frame) => {
  ...
  }, [auth.user, chat.messages, dispatch]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="border border-slate-600/50 rounded-3xl shadow-2xl bg-slate-800/60 backdrop-blur-xl overflow-hidden h-full flex flex-col">
        ...
        <ScrollArea className="flex-1 w-full p-6 flex gap-4 flex-col bg-gradient-to-b from-transparent to-slate-900/20 overflow-y-auto">
          {chat.chat?.map((item, i) =>
            item.sender.id === auth.user.id ? (
              <div
                key={i}
                ref={chatContainerRef}
                className="flex gap-4 mb-4 animate-fade-in group"
              >
